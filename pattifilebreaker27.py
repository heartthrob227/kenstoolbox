...
filename = raw_input('What file will we be looking at (CSV format required; no extension)? ')
filenamewithtext = filename + '.csv'
fout = open(filename + '_filtered.csv', 'ab+')
...
with open(filenamewithtext) as file:
    ...
    writer.writeheader()
    column = raw_input('What column are we filtering? ')
    filtervalue = raw_input('What is the filtering criteria (comma separated)? ')
